  ...
  <script>
    $(document).ready(function() {
      var dataPengaduan = $('#table_pengaduan').DataTable({
        ...
        ajax: {
          url: "{{ route('data.pengaduan_list') }}",
          dataType: 'json',
          type: 'POST',
        },
        columns: [
            { data: 'DT_RowIndex', orderable: true, searchable: false, className: "text-center py-3 asap border-t-2" },
            { data: 'tanggal_pengaduan', orderable: true, searchable: true, className: "text-center py-3 asap border-t-2" },
            { data: 'nama', orderable: false, searchable: true, className: "text-center asap border-t-2" },

...
<main class="w-full h-full">
    <div class="flex flex-col items-center py-10 min-w-fit">
        <div class="w-5/6 bg-primary p-3 text-white rounded-t-xl border-2 border-b-0 font-bold flex flex-row min-w-[490px]">
            <a class="bg-white rounded-full size-7 flex justify-center items-center" href="{{ url('rt_notifikasi') }}">
        ...
        <div class="w-5/6 flex flex-col items-center border-2 border-t-2">
        ...
        <div class="rounded-b-xl border-2 w-5/6 flex justify-center p-3 gap-x-3 text-white font-semibold" style="font-family: Asap">
            <form action="{{ route('bansos.update', ['id' => $bansos->id_bansos]) }}" method="POST" class="w-full" id="form-ditolak">
                @csrf
                <input type="hidden" name="status" value="ditolak">
                <button type="submit" class="bg-red-600 w-full p-3 rounded-xl">Data Tidak Valid</button>
            </form>
            <form action="{{ route('bansos.update', ['id' => $bansos->id_bansos]) }}" method="POST" class="w-full" id="form-diterima">
                @csrf
                <input type="hidden" name="status" value="diterima">
                <button type="submit" class="bg-primary w-full p-3 rounded-xl">Data Valid</button>
            </form>
        </div>
    </div>
</main>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('form-ditolak').addEventListener('submit', function(event) {
        ...
        document.getElementById('form-diterima').addEventListener('submit', function(event) {
            event.preventDefault();
            Swal.fire({
                title: 'Apakah Anda yakin?',
